export default {
    accounts: {
      ...
      zcode: {
        ...
        apiKeyOptional: '可选 —— OAuth 登录后自动填充',
        apiKeyHint: 'OAuth 登录后自动填充，也可手动填入 z.ai / 智谱 Coding Plan 的 API Key（zai 格式为 {key}.{secret}）。',
        jwtRequired: 'Start Plan 需要先 OAuth 登录获取 plan JWT',
        oauth: {
          title: 'ZCode OAuth 登录',
          desc: '使用 Z.AI / 智谱账号授权，自动创建 Coding Plan API Key。登录流量可走所选代理。',
          start: '开始登录',
          running: '等待中…',
          ...
          complete: '完成登录',
          waitingPoll: '等待授权中……本页会自动轮询。',
          waitingPaste: '授权后浏览器会跳转到 127.0.0.1（无法打开）——从地址栏复制该 URL 并粘贴到这里。',
          success: '登录成功 —— 凭据已自动填充。',
          failed: '授权失败，请重试。'
        }
      },
